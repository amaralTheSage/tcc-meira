...
export function DocsContextMenu({ children, onAddText, onAddCode, onAddDivider, onAddCallout, onAddImage, onAddList }: DocsContextMenuProps) {
    ...
    return (
        <ContextMenu>
            ...
            <ContextMenuContent>
                {menuItems.map((item) => (
                    <ContextMenuItem key={item.label} inset onSelect={item.action} className="mx-0 w-fit w-full px-10 !pl-4">
                        <item.icon className="mr-2 h-4 w-4" />
                        {item.label}
                    </ContextMenuItem>
                ))}
                <ContextMenuSub>
                    <ContextMenuSubTrigger inset className="mx-0 w-fit w-full px-10 !pl-4">
                    ...
                    <ContextMenuSubContent>
                        {calloutVariants.map((variant) => (
                            <ContextMenuItem
                                key={variant.type}
                                inset
                                onSelect={() => onAddCallout(variant.type)}
                                className="mx-0 w-fit w-full px-10 !pl-4"
                            >
